...
// Sync mobile menu buttons with header buttons
function syncMobileMenuButtons() {
  ...
  function updateMobileMenuVisibility() {
    ...
    if (userBadge && mobileUserBadge) {
      if (userBadge.classList.contains('hidden')) {
        mobileUserBadge.classList.add('hidden');
      } else {
        mobileUserBadge.classList.remove('hidden');
        mobileUserBadge.textContent = userBadge.textContent;
      }
    }
    
    // Update theme button text - always visible
    if (mobileThemeToggle) {
      const isDark = document.body.classList.contains('dark');
      mobileThemeToggle.textContent = isDark ? '◐ Theme' : '◉ Theme';
      mobileThemeToggle.classList.remove('hidden');
    }
  ...
  if (mobileRegisterBtn && registerBtn) {
  ...
  if (mobileAccountBtn && accountBtn) {
  ...
  if (mobileThemeToggle && themeToggle) {
    mobileThemeToggle.addEventListener('click', () => {
      themeToggle.click();
      // Update text after theme change
      setTimeout(() => {
        const isDark = document.body.classList.contains('dark');
        mobileThemeToggle.textContent = isDark ? '◐ Theme' : '◉ Theme';
      }, 100);
  ...
  if (userBadge && mobileUserBadge) {
    ...
    if (userBadge) observer.observe(userBadge, { attributes: true, attributeFilter: ['class'] });
    if (loginBtn) observer.observe(loginBtn, { attributes: true, attributeFilter: ['class'] });
    if (logoutBtn) observer.observe(logoutBtn, { attributes: true, attributeFilter: ['class'] });
    if (themeToggle) {
...
updateProgress();
initCalculator();
// Sync mobile menu after auth UI is updated
setTimeout(() => {
